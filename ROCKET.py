'''
Project Name: R.O.C.K.E.T. (Rocket Optimization Control and Kinematics Evaluation Tool)

Project Description:
This project aims to create a sophisticated Rocket Simulation software, capable of computing
the most fuel efficient trajectory for Solar System interplanetary travel, with the 
"Hohmann Transfer Orbit".  By considering various factors such as the gravitational pull of the
planets and their orbits, this software will enable users to identify the optimal flight path
between two planets while minimizing fuel consumption and travel time.

Team members: Suraj Vaidyanathan, Adarsh K Prasad, Parv D Parekh

Date: 01.Nov.23

Module Name: ROCKET.py

Module Description: 
This is the main module which implements core logic.  TKINTER is the front-end GUI package and 
mySQL is the backend database.
'''

import launchDate # user defined module which implements launch date calculation
import matplotlib
import matplotlib.animation as animation
import db #user defined module to connect to mySQL
...
matplotlib.use('TkAgg') #Optional argument to ensure code works on macOS
...
class Planet():
    '''
        The planet class contains the physics to calculate
        the orbital motion of the planet around the central body
    '''
    ...
    def __init__(self, a, T, e, m, sim, c):
        ...
        self.eccentricAnomaly = None    # Receives angle in radians on initialisation
        self.position = None            # Given as (x, y) coordinate on initialisation
        self.velocity = None            # Given as (vx, vy) components on initialisation
    ...
    def calculateInitialPosition(self):
        # Use distance and anomaly to calculate initial positions of planet x0, y0
...
def get_planet_data(name, property) -> [float, str]:
    # Retrieves all data on planets from SQL Database
    data = db.retrieve(name)
...
class SolarSystemSimulation():
    ...
    def __init__(self):
        ...
        self.delta = gui.stepsize.get() * 2*np.pi / (np.sqrt(G*M) * 365.256)   # Integration timestep in Earth days (determines accuracy)
        self.steps = round(365.256 * gui.duration.get() / gui.stepsize.get())  # Integration time in Earth years (determines duration of simulation)
        ...
        Mercury = Planet(a = get_planet_data('mercury', 'semimajor'), 
                         e = get_planet_data('mercury', 'eccentricity'), 
                         T = get_planet_data('mercury', 'orbitalperiod'), 
                         m = get_planet_data('mercury', 'mass'),     
                         sim=gui.mercury.get(), 
                         c = get_planet_data('mercury', 'color')
                         )
        Venus   = Planet(a = get_planet_data('venus', 'semimajor'), 
                         e = get_planet_data('venus', 'eccentricity'), 
                         T = get_planet_data('venus', 'orbitalperiod'), 
                         m = get_planet_data('venus', 'mass'),     
                         sim=gui.venus.get(), 
                         c = get_planet_data('venus', 'color')
                         )
        Earth   = Planet(a = get_planet_data('earth', 'semimajor'), 
                         e = get_planet_data('earth', 'eccentricity'), 
                         T = get_planet_data('earth', 'orbitalperiod'), 
                         m = get_planet_data('earth', 'mass'),     
                         sim=gui.earth.get(), 
                         c = get_planet_data('earth', 'color')
                         )
        Mars    = Planet(a = get_planet_data('mars', 'semimajor'), 
                         e = get_planet_data('mars', 'eccentricity'), 
                         T = get_planet_data('mars', 'orbitalperiod'), 
                         m = get_planet_data('mars', 'mass'),     
                         sim=gui.mars.get(), 
                         c = get_planet_data('mars', 'color')
                         )
        Jupiter = Planet(a = get_planet_data('jupiter', 'semimajor'), 
                         e = get_planet_data('jupiter', 'eccentricity'), 
                         T = get_planet_data('jupiter', 'orbitalperiod'), 
                         m = get_planet_data('jupiter', 'mass'),     
                         sim=gui.jupiter.get(), 
                         c = get_planet_data('jupiter', 'color')
                         )
        Saturn  = Planet(a = get_planet_data('saturn', 'semimajor'), 
                         e = get_planet_data('saturn', 'eccentricity'), 
                         T = get_planet_data('saturn', 'orbitalperiod'), 
                         m = get_planet_data('saturn', 'mass'),     
                         sim=gui.saturn.get(), 
                         c = get_planet_data('saturn', 'color')
                         )
        Uranus  = Planet(a = get_planet_data('uranus', 'semimajor'), 
                         e = get_planet_data('uranus', 'eccentricity'), 
                         T = get_planet_data('uranus', 'orbitalperiod'), 
                         m = get_planet_data('uranus', 'mass'),     
                         sim=gui.uranus.get(), 
                         c = get_planet_data('uranus', 'color')
                         )
        Neptune = Planet(a = get_planet_data('neptune', 'semimajor'), 
                         e = get_planet_data('neptune', 'eccentricity'), 
                         T = get_planet_data('neptune', 'orbitalperiod'), 
                         m = get_planet_data('neptune', 'mass'),     
                         sim=gui.neptune.get(), 
                         c = get_planet_data('neptune', 'color')
                         )
...
class App:
    ...
    def __init__(self, master):
        ...
        self.quit.grid(column=2, row=13, sticky='EW')
        ...
        self.transfer_frame.grid(row=5, columnspan=3, sticky='EW')
        ...
        self.origin.bind('<<ListboxSelect>>',self.callback) # Catches changes made to origin planet
        ...
        self.destination.bind('<<ListboxSelect>>',self.callback) # Catches changes made to destination planet
        ...
        #self.suggestionButton = ttk.Button(master, text='Calculate Simulation Parameters', command=self.suggestSimParameters)
        #self.suggestionButton.grid(column=0, row=9, sticky='EW')
        ...
        tk.Entry(self.transfer_frame, textvariable=self.stepsize).grid(row=10, column=2, sticky='EW')
        tk.Label(self.transfer_frame, text='Timestep in Earth days (For Accuracy)').grid(row=10, column=0, sticky='W')

        self.duration = tk.DoubleVar()
        tk.Entry(self.transfer_frame, textvariable=self.duration).grid(row=11, column=2, sticky='EW')
        tk.Label(self.transfer_frame, text='Integration Time in Earth Years (Duration of Simulation)').grid(row=11, column=0, sticky='E')

        self.plot_button = ttk.Button(master, text='Plot', command=self.plot)
        self.plot_button.grid(column=1, row=9, sticky='EW')

        self.animate_button = ttk.Button(master, text='Animate', command=self.simulation_animation)
        self.animate_button.grid(column=2, row=9, sticky='EW')

        self.movie_button = ttk.Button(master, text='Save as mp4', command=self.movie)
        self.movie_button.grid(column=0, row=9, sticky='EW')
    ...
    def plot(self):
        # Checks if origin and destination planets are selected
        selection1=gui.origin.curselection()
        selection2=gui.destination.curselection()
        ...
        if selection1 and selection2:
            ...
            sim_window.resizable(False,False) # Prevent going to fullscreen
            def on_closing(): # Quit the simulation on closing
                sim_window.destroy()
            ...
            if gui.plotHohmann.get():
                ...
                originPos,destPos = tuple(),tuple()
                for key,planet in Sim.Planets.items():
                    if key == Sim.originPlanet:
                        originPos = planet.calculateInitialPosition()
                    elif key == Sim.destinationPlanet:
                        destPos = planet.calculateInitialPosition()
                unit_origin = originPos / np.linalg.norm(originPos)
                unit_dest = destPos / np.linalg.norm(destPos)
                theta = (np.arccos(np.dot(unit_origin, unit_dest)))
                info['launchDate'].set_text("Launch Date: " + launchDate.calculate(theta, Sim.originPlanet, Sim.destinationPlanet))
            ...
            for key, planet in Sim.Planets.items():
                ...
                ax.plot(*planetTrajectories[key][-1], marker=planet.marker, c=planet.colour, label=key)

            # Add Sun
            sun, = ax.plot((0, 0), c='orange', marker='o', ls='')

            legend = ax.legend(loc=3, labelcolor='w')
            ...
            # If no planets are selected, display error message
            self.flag = True
            self.label = tk.Label(self.transfer_frame, text='Select origin and destination planets!!!', fg='red')
    ...
    def simulation_animation(self):
        ...
        if selection1 and selection2:
            ...
            if self.flag==True:  self.label.destroy()
            # ------
            # Start simulation
            Sim = SolarSystemSimulation()
            ...
            for key, planet in Sim.Planets.items():
                ...
                sphere, = ax.plot([], [], marker=planet.marker, c=planet.colour, label=key)
            ...
            def animate(i):
                ...
                if gui.plotHohmann.get():
                    ...
                    if distancesToSpacecraft[Sim.destinationPlanet] <= sphereOfInfluence:
                        # The HTO succeeded
                        info['transferStatus'].set_text('Transfer Status: in progress..complete')
    ...
    def addInformation(self, ax):
        ...
        info['originDestination'] = ax.text(0.45, 0.95, '', transform=ax.transAxes, color='white')
        info['transferTime'] = ax.text(0.01, 0.93, '', transform=ax.transAxes, color='white')
        info['transferDistance'] = ax.text(0.01, 0.91, '', transform=ax.transAxes, color='white')
        info['elapsedTime'] = ax.text(0.65, 0.93, '', transform=ax.transAxes, color='white')
        info['destinationDistance'] = ax.text(0.65, 0.91, '', transform=ax.transAxes, color='white')
        info['transferStatus'] = ax.text(0.65, 0.89, '', transform=ax.transAxes, color='white')
        info['launchDate'] = ax.text(0.01, 0.89, '', transform=ax.transAxes, color='white')
        return info
    ...
    def update(self,*args):
        # Changes color of checklist to gray or to black according to whether planet is selected
        for key in self.check_dict:
            if self.check_dict[key].get()==False:
                self.origin.itemconfig(key, fg="gray")
                self.destination.itemconfig(key,fg='gray')
                # Clears selection if it was previously selected but later unchecked from display list
                selection1=self.origin.curselection()
                selection2=self.destination.curselection()
                if selection1:
                    if selection1[0]==key:  self.origin.select_clear(key)
                if selection2:  
                    if selection2[0]==key:  self.destination.select_clear(key)
            else:
                self.origin.itemconfig(key,fg='black')
                self.destination.itemconfig(key,fg='black')
    
    def callback(self,event):
        selection = event.widget.curselection()
        if selection and self.check_dict[selection[0]].get()==False:
            index = selection[0]
            self.origin.select_clear(index)
            self.destination.selection_clear(index)
        selection1 = gui.origin.curselection()
        selection2 = gui.destination.curselection()
        if selection1 and selection2: # Checks if planets are selected and calculates parameters
            self.suggestSimParameters()
...
if __name__ == '__main__':
    ...
    # Start GUI
    root = tk.Tk()
    root.wm_title('R.O.C.K.E.T.')
    root.resizable(False, False)
    gui = App(root)

    # Catches changes made in planet display
    for planet in gui.check_dict:
        gui.check_dict[planet].trace('w',gui.update)
